const rawFingerings = {
  ...
  // Normalize fingering strings to canonical comma-separated digits or null
  function normalizeFingering(raw) {
    if (!raw || typeof raw !== 'string') return null;
    const firstAlt = raw.split(',')[0].trim();
    const lower = firstAlt.toLowerCase();
    if (lower.includes('out of bound') || lower.includes('out of')) return null;
    let s = firstAlt.replace(/\+/g, ',');
    s = s.replace(/[^0-9,]/g, '');
